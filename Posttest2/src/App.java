...
public class App {
    ...
    private static void readData(ArrayList<Bus> buses) {
        if (buses.isEmpty()) {
            ...
            for (int i = 0; i < buses.size(); i++) {
                Bus bus = buses.get(i);
                System.out.printf("%-4d%-15s%-25s%-15d%-15d%-15d%n", i + 1, bus.getNama(), bus.getFasilitas(), bus.getKapasitas(), bus.getHargaSewa(), bus.getJumlahUnit());
            }
        }
    }
    ...
    private static void createData(ArrayList<Bus> buses, Scanner scanner) {
        ...
        bus.setFasilitas(scanner.nextLine());
    ...
    private static int getIndexToUpdate(ArrayList<Bus> buses, Scanner scanner) {
        ...
        for (int i = 0; i < buses.size(); i++) {
            if (buses.get(i).getNama().equals(nama)) {
                return i;
            }
        }
