    ...
    private static void readDataRegular(ArrayList<BusRegular> regularBuses) {
        if (regularBuses.isEmpty()) {
            ...
            for (int i = 0; i < regularBuses.size(); i++) {
                ...
                System.out.printf("%-6d%-15s%-15d%-15d%-15d%-15s%n", i + 1, bus.getNama(), bus.getKapasitas(),
                        bus.getHargaSewa(), bus.getJumlahUnit(), bus.isMusik() ? "Ya" : "Tidak");
            }
    ...
    private static void readDataVIP(ArrayList<BusVIP> VIPBuses) {
        if (VIPBuses.isEmpty()) {
            ...
            System.out.printf("%-6s%-15s%-15s%-15s%-15s%-15s%-15s%n", "No.", "Nama Bus", "Kapasitas", "Harga Sewa",
                    "Jumlah Unit", "Pemandu", "Wifi");
            System.out.println("=====================================================================================");

            for (int i = 0; i < VIPBuses.size(); i++) {
                BusVIP bus = VIPBuses.get(i);
                System.out.printf("%-6d%-15s%-15d%-15d%-15d%-15s%-15s%n", i + 1, bus.getNama(), bus.getKapasitas(),
                        bus.getHargaSewa(), bus.getJumlahUnit(), bus.isPemandu() ? "Ya" : "Tidak",
                        bus.isWifi() ? "Ya" : "Tidak");
            }
        }
    }
